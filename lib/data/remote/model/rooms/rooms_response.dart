...
List<RoomsResponse> roomsResponseFromJson(String str) =>
    List<RoomsResponse>.from(json.decode(str)["L"].map((x) => RoomsResponse.fromJson(x["M"])));

String roomsResponseToJson(List<RoomsResponse> data) =>
    json.encode({"L": List<dynamic>.from(data.map((x) => {"M": x.toJson()}))});
